...
public class TopLevelClass extends InnerClass implements CompilationUnit {
    ...
    private void sortImportsType(Set<String> importStrings, StringBuilder sb) {
        Set<String> javaImports = new HashSet<String>();
        Set<String> orgImports = new HashSet<String>();
        Set<String> otherImports = new HashSet<String>();
        Set<String> comYcImports = new HashSet<String>();
        for (String importStr : importStrings) {
            String importTypeStr = importStr.substring("import ".length());
            if (importTypeStr.startsWith("java")) {
                javaImports.add(importStr);
            }
            else if (importTypeStr.startsWith("org")) {
                orgImports.add(importStr);
            }
            else if (importTypeStr.startsWith("com.yc")) {
                comYcImports.add(importStr);
            }
            else {
        ...
        for (String importStr : javaImports) {
            sb.append(importStr);
        }
        ...
        for (String importStr : orgImports) {
            sb.append(importStr);
        }
        
        if (orgImports.size() > 0) {
            newLine(sb);
        }
        
        for (String importStr : otherImports) {
            sb.append(importStr);
        }
        ...
        for (String importStr : comYcImports) {
            sb.append(importStr);
        }
        
        if (comYcImports.size() > 0) {
            newLine(sb);
        }
    }
