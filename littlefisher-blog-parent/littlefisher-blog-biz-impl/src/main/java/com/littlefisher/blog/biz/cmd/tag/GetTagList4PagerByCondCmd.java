...
public class GetTagList4PagerByCondCmd extends AbstractCommand<PageInfo<TagBizDto>> {
    ...
    public PageInfo<TagBizDto> execute() {
        PageInfo<TagDto> pageInfo
            = tagDtoDao.selectTag4PageByCond(SelectTag4PageByCondParam.Builder.getInstance().build(), PageParam.Builder
                .getInstance().addPageNum(request.getPageNum()).addPageSize(request.getPageSize()).build());
